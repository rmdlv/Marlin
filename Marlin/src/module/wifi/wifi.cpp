...
bool once_flag, flash_preview_begin, default_preview_flg, gcode_preview_over;
...
//       PGM_P p = PSTR("|<>^+=?/[];,*\"\\");
//       while (uint8_t b = pgm_read_byte(p++)) if (b == c) continue;
//       if (c < 0x21 || c == 0x7F) continue;                        // Check size, non-printable characters
...
int package_to_wifi(WIFI_RET_TYPE type, uint8_t *buf, int len) {
  ...
  if (type == WIFI_PARA_SET) {
    int data_offset = 4,
        apLen = strlen((const char *)wifi_cfg.wifi_name),
        keyLen = strlen((const char *)wifi_cfg.wifi_key);

    ZERO(buf_to_wifi);
    index_to_wifi = 0;

    buf_to_wifi[data_offset] = wifi_cfg.wifi_mode_sel;
    buf_to_wifi[data_offset + 1]  = apLen;
    memcpy(&buf_to_wifi[data_offset + 2], (const char *)wifi_cfg.wifi_name, apLen);
    buf_to_wifi[data_offset + apLen + 2]  = keyLen;
    memcpy(&buf_to_wifi[data_offset + apLen + 3], (const char *)wifi_cfg.wifi_key, keyLen);
...
typedef struct {
  int write_index;
  uint8_t saveFileName[LONG_FILENAME_LENGTH];
...
char saveFilePath[LONG_FILENAME_LENGTH];
...
int write_to_file(char *buf, int len) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (file_writer.write_index >= 512) {
      ...
      mks_update_status(pos.position, file_writer.fileLen);
  ...
  if (strchr((char *)cmd_line, '\n') && (strchr((char *)cmd_line, 'G') || strchr((char *)cmd_line, 'M') || strchr((char *)cmd_line, 'T'))) {
    ...
    if (tmpStr) {
      ...
      switch (cmd_value) {
          ...
          if (!card.isPrinting() || !card.isPaused()) {
            ...
            if (strstr_P((char *)&tmpStr[index], PSTR(".g")) || strstr_P((char *)&tmpStr[index], PSTR(".G"))) {
              if (strlen((char *)&tmpStr[index]) < 80) {
                ...
                // if (strncmp_P((char *)&tmpStr[index], PSTR("1:"), 2) == 0 || strncmp_P((char *)&tmpStr[index], PSTR("0:"), 2) == 0) {
                //   has_path_selected = 1;
                // }
                // else
                if (tmpStr[index] != '/') strcat_P((char *)list_file.file_name[sel_id], PSTR("/"));
        ...
        case 105:
        case 991: // M991：like M105, get the temperature
          ZERO(tempBuf);
          if (cmd_value == 105) {
...
static void file_first_msg_handle(uint8_t * msg, uint16_t msgLen) {
  ...
  if (strlen((const char *)file_writer.saveFileName) > sizeof(saveFilePath)) return;

  ZERO(public_buf);

  // draw UI

  mks_upload_screen((char*)file_writer.saveFileName);
